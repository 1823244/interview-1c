...
```
Это была ОПТИМИСТИЧЕСКАЯ блокировка (ее сообщение об ошибке)  
...
Устанавливает ТРАНЗАКЦИОННУЮ блокировку!  
И вообще, объект Блокировка - это Управляемая Транзакционная Блокировка!  
https://its.1c.ru/db/v837doc#bookmark:dev:TI000000524  

НЕЛЬЗЯ использовать без транзакции!  
...
Можно использовать без транзакции.  
	...
	Для Каждого Строка Из ТабЗнач Цикл
		...
		Пока НЕ ОбъектОбработан Цикл
			...
			Попытка
				...
				//КонтОб.Заблокировать();
				//добавить
				ЗаблокироватьДанныеДляРедактирования(КонтСс);
				//перенести после блокировки
				КонтОб = КонтСс.получитьОБъект();  
				контОб.наименование = "клиент 2 - "+строка(текущаяДата()); 
				контОб.Записать();  
				СУУ_УниверсальныеСервер.Пауза(10);
